      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _FeaturedStoreCard(item: featured, apiBaseUrl: apiBaseUrl),
...
class _FeaturedStoreCard extends StatelessWidget {
  const _FeaturedStoreCard({required this.item, required this.apiBaseUrl});
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final image = _StoreImage(
          imageUrl: imageUrl,
          icon: _storeIconForContent(item.contentType),
          height: compact ? 230 : 310,
          featured: true,
        );
        final info = _StoreInfo(item: item, featured: true);
        return Container(
          decoration: BoxDecoration(
            gradient: const LinearGradient(
              begin: Alignment.topLeft,
              end: Alignment.bottomRight,
              colors: [Color(0xFF06142D), Color(0xFF0B2F6B), Color(0xFF0EA5E9)],
            ),
            borderRadius: BorderRadius.circular(18),
          ),
          child: Padding(
            ...
            child: compact
                ? Column(
                    ...
                    children: [
                      ...
                      _StoreButton(item: item, hasUrl: hasUrl, featured: true),
                    ],
                  )
                : Row(
                    children: [
  ...
  Widget build(BuildContext context) {
    return ClipRRect(
      ...
      child: Container(
        ...
        color: featured ? Colors.white : const Color(0xFFEAF2FF),
        child: imageUrl == null
            ? Icon(
                icon,
                size: featured ? 84 : 42,
...
class _StoreInfo extends StatelessWidget {
  const _StoreInfo({required this.item, this.featured = false});
  ...
  @override
  Widget build(BuildContext context) {
    final foreground = featured ? Colors.white : const Color(0xFF0F172A);
    final muted = featured ? const Color(0xFFDCEBFF) : const Color(0xFF64748B);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if ((item.badge ?? '').isNotEmpty) ...[
          _AlertBadge(
            label: item.badge!,
            color: featured ? const Color(0xFF38BDF8) : const Color(0xFF2563EB),
          ),
          const SizedBox(height: 10),
        ],
        Text(
...
class _StoreButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final label = (item.buttonLabel ?? '').isNotEmpty
        ...
        ? 'Abrir oferta'
        : 'Comprar agora';
    final onPressed = hasUrl ? () => redirectToUrl(item.targetUrl!) : null;
    if (featured) {
